...
def get_train():
    ...
    ica = joblib.load("../data/model/ica.joblib")
    count = 0

    for i, element in enumerate(file):
        element = ica.transform(element)
    ...
    new.to_csv('ica.csv', header=None, index=None)


# u = u.T  # shape=[4,200]

def get_test():
    pca = joblib.load("../data/model/ica.joblib")
    file = get_file(False)
    count = 0
    for i, element in enumerate(file):
    ...
    new = train.replace({'out': normal_dic})
    new.to_csv('ica_test.csv', header=None, index=None)
