...
from dataprocess.pca import get_file
...
def get_train():
    ...
    cca = joblib.load('../data/model/cca.joblib')
    # model = VAMP(lagtime=3, dim=9).fit(train.values).fetch_model()
    # joblib.dump(model, '../data/model/cca.joblib')
    count = 0
    for i, element in enumerate(file):
        start_time = datetime.datetime.now()
        element = cca.transform(element)
    ...
    new.to_csv('cca.csv', header=None, index=None)


def get_test():
    pca = joblib.load("../data/model/cca.joblib")
    file = get_file(False)
    count = 0
    for i, element in enumerate(file):
    ...
    new = train.replace({'out': normal_dic})
    new.to_csv('cca_test.csv', header=None, index=None)
